...
# If $directory is omitted, creates `BonnAnalysis.lean`.

cd "$( dirname "${BASH_SOURCE[0]}" )"/../BonnAnalysis
if [[ $# = 1 ]]; then
...
# prepend `import «`
find "$dir" -name \*.lean -not -name BonnAnalysis.lean \
  | sed 's,^\./,,;s,^\.\./[^/]*,,;s,/,.,g;s,\.lean$,,;s,^,import BonnAnalysis.,' \
  | sort >"$dir"/../BonnAnalysis.lean
